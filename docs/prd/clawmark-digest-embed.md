# ClawMark 嵌入 Sliver PRD

## 背景

Sliver 在 staging 环境测试时，测试人员发现 bug 需要手动截图、描述问题、发到群里。流程低效且容易丢失上下文（当前页面 URL、截图、操作路径等）。

ClawMark（collectivewinca/clawmark）是已开源的反馈/标注组件，提供 Fab 插件（浮动操作按钮）。将 ClawMark 嵌入 Sliver 应用，让测试人员可以直接在页面上提交 bug 报告，附带自动采集的上下文信息。

## 方案

### 设计

在 Sliver 所有页面嵌入 ClawMark Fab 插件（浮动按钮），点击后弹出反馈表单。

#### 用户交互流程

1. 用户在 Sliver 任意页面看到右下角浮动按钮
2. 点击按钮 → 弹出反馈表单
3. 填写：bug 描述 + 可选截图/标注
...
1. **ClawMark server 部署**：独立进程部署（Kevin 确认），域名由 Lisa 配置（如 clawmark.kevinhe.io）。使用 collectivewinca/clawmark 仓库自带的 Express + SQLite 后端。
2. **前端嵌入**：在 Sliver 的 HTML 模板中引入 ClawMark SDK（`<script>` 标签），初始化 Fab 插件，指向 ClawMark server
3. **自动采集**：当前页面 URL、浏览器 UA、用户登录状态（如有）自动附带在反馈中
4. **线上服务化**：ClawMark server 作为通用服务运行，不仅服务 Sliver，其他项目也可接入（开源集成 vs 在线集成）
...
- Sliver HTML 模板 — 新增 `<script>` 引用 + 初始化代码
...
1. [ ] Sliver 页面右下角显示 ClawMark 浮动按钮
2. [ ] 点击按钮弹出反馈表单
3. [ ] 填写描述并提交成功
4. [ ] 提交的反馈包含当前页面 URL
5. [ ] 提交后可选通知到 Lark 群
6. [ ] `CLAWMARK_ENABLED=false` 时按钮不显示
7. [ ] 不影响 Sliver 已有功能（digest 列表、登录、sources）
...
| # | 场景 | 步骤 | 预期结果 |
|---|------|------|----------|
| 1 | Fab 按钮可见 | 打开 Sliver 任意页面 | 右下角可见浮动按钮 |
...
| 8 | 移动端 | 手机浏览器打开 Sliver | 浮动按钮可见、可点击、不遮挡内容 |
| 9 | ClawMark 服务不可用 | ClawMark server 关闭 → 打开 Sliver | Sliver 正常加载，Fab 按钮可能不可用但不影响主功能 |
